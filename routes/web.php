...
Route::get('/report-checkout-item/{stock_id}', [ReportStockController::class,'index'])
    ->name('report-checkout-item')
    ->middleware('auth','can:view_master_data');
Route::get('/get-checkout-item/{stock_id}/{year}/{month}', [ReportStockController::class,'show'])
    ->name('get-checkout-item')
    ->middleware('auth');
Route::get('/export-checkout-item/{stock_id}/{year}/{month}', [ReportStockController::class,'export'])
    ->name('export-checkout-item')
    ->middleware('auth');
Route::get('/export-checkout-item-test/{checkout_items}', [ReportStockController::class,'export_test'])
    ->name('export-checkout-item-test')
    ->middleware('auth');
Route::get('/print-cutstock/pdf/{stock_id}/{year}/{month}', [PrintFormController::class,'printCutStock'])
    ->name('print-cutstock-pdf')
    ->middleware('auth');
Route::get('/stockitem/viewpdf/{stock_id}/{year}/{month}', [PDFController::class, 'viewPDF'])
    ->name('view-cutstockitem-pdf')
    ->middleware('auth');
Route::get('/export-balance-stock/{stock_id}', [ReportStockController::class,'exportBalanceStock'])
    ->name('export-balance-stock')
    ->middleware('auth');
...
Route::match(['get', 'post'],'/admin/report-list/{division_id}',[AdminReportStockController::class,'index'])
    ->name('report-list')
    ->middleware(['auth','remember']);
Route::post('/admin/report-stock', [AdminReportStockController::class,'show'])
...
//อนุมัติใบสั่งซื้อ
Route::post('/admin/order-list/update', [AdminOrderController::class,'update'])
    ->name('approve-order')
    ->middleware('auth','can:manage_master_data');

//แสดงหน้าแรก ค้นหาข้อมูลงบประมาณในแต่ละปี
Route::get('/admin/budget-list/', [BudgetController::class,'index'])
    ->name('budget-list')
    ->middleware('auth','can:view_master_data');
//ดึงรายการงบประมาณตั้งต้นแต่ละสาขา ตามปีที่ระบุ
Route::get('/admin/get-list-budget', [BudgetController::class,'show'])
    ->name('get-list-budget')
    ->middleware('auth','can:view_master_data')
    ->middleware('remember');
//บันทึกงบประมาณสาขา
Route::post('/admin/add-budget',[BudgetController::class,'store'])
    ->name('add-budget')
    ->middleware('auth','can:manage_master_data');
//แก้ไขข้อมูลงบ
Route::post('/admin/edit-budget',[BudgetController::class,'edit'])
    ->name('edit-budget')
    ->middleware('auth','can:manage_master_data');
//ดึงรายการการเปลี่ยนแปลงของงบประมาณสาขานั้นปีนั้น
Route::get('/get-budget-log/{budget}/log',[ResourceActionLogController::class,'showLogBudget'])
    ->name('get-budget-log')
    ->middleware('auth','can:manage_master_data');

//พิมพ์งบประมาณคงเหลือและใบสั่งซื้อ
Route::get('/admin/print-budget-order/{stock_id}/{year}', [PrintFormController::class,'printBudgetOrder'])
    ->name('print-budget-order')
    ->middleware('auth','can:view_master_data');
//แสดงงบประมาณในรายการสั่งซื้อแต่ละครั้ง
Route::get('/admin/get-list-order/{stock_id}/{year}', [ItemTransactionController::class,'index'])
    ->name('get-list-order')
    ->middleware(['auth','can:view_master_data','remember']);
//พิมพ์งบประมาณคงเหลือและใบสั่งซื้อ
Route::get('/admin/print-budget-order-import/{stock_id}/{year}', [PrintFormController::class,'printBudgetOrderImport'])
    ->name('print-budget-order-import')
    ->middleware('auth','can:view_master_data');
//ดึงรายรายละเอียดการสั่งซื้อ ตามเลข Pur.Order
Route::post('/admin/get-list-budget-detail', [ItemTransactionController::class,'edit'])
    ->name('get-list-budget-detail')
    ->middleware('auth','can:view_master_data');
...
//พิมพ์ใบสั่งซื้อ
Route::get('/purchase-order/print/{order}', [PrintFormController::class,'printPurchaseOrder'])
    ->name('print-purchase-order')
    ->middleware('auth');
Route::get('/purchase-order/print-item/{order}', [PrintFormController::class,'printPurchaseOrderItem'])
    ->name('print-purchase-order-item')
    ->middleware('auth');

//แสดงหน้าตรวจรับวัสดุ จากใบสั่งซื้อ
Route::get('/receive-order-purchase/{order}', [CheckInOrderPurchaseController::class,'show'])
    ->name('receive-order-purchase')
    ->middleware('auth');

//ค้นหาวัสดุ
Route::get('/search-stock-item/{item_name_search}', [StockItemController::class,'searchByItemName'])
    ->name('search-stock-item')
    ->middleware('auth');
//printForm test
Route::get('/testprint', [PrintFormController::class,'index'])->name('testprint');

//Import Data From Excel
Route::get('/stockitem/import',[StockItemImportController::class,'index'])
    ->name('stock-item-import')
    ->middleware('auth')
    ->middleware('remember');

Route::post('/stockitem/import',[StockItemImportController::class,'show'])
    ->name('stock-item-import-show')
    ->middleware('auth');

Route::post('/stockitem/checkin-to-stock',[StockItemImportController::class,'store'])
    ->name('import-checkin-to-stock')
    ->middleware('auth');
...
Route::get('/user/add',[UserController::class,'index'])->name('user-add')
    ->middleware('auth')
    ->middleware('remember');
Route::get('/user/check-employee-status/{sap_id}',[UserController::class,'checkEmployeeStatus'])
...
Route::get('/user/show-detail-user/{slug}',[UserController::class,'edit'])
    ->name('show-detail-user')
    ->middleware('auth');
//update stock
Route::post('/user/update-user/{user}',[UserController::class,'update'])
    ->name('update-user')
    ->middleware('auth');
//ดึงรายการการเปลี่ยนแปลงของ user
Route::get('/get-user-log/{user}/log',[ResourceActionLogController::class,'showLogUser'])
    ->name('get-user-log')
    ->middleware('auth','can:manage_master_data');
